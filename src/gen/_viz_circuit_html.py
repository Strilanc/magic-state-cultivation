...
from gen._chunk import ChunkInterface, StabilizerCode, Patch, Tile, find_d2_error
...
class _SvgLayer:
    def __init__(
        ...
        tile_color_func: Callable[
            [Tile], tuple[float, float, float, float] | tuple[float, float, float] | str
        ],
    ):
    ...
    def add_patch(
        ...
        for tile in patch.tiles:
            ...
            if isinstance(color, tuple):
                ...
                color = (
                    "#"
                    + hex(round(r * 255.49))[2:].rjust(2, "0")
                    + hex(round(g * 255.49))[2:].rjust(2, "0")
                    + hex(round(b * 255.49))[2:].rjust(2, "0")
                    + hex(round(a * 255.49))[2:].rjust(2, "0")
                )
...
class _SvgState:
    def __init__(
        ...
        self.layers: list[_SvgLayer] = [
            _SvgLayer(self.patch, tile_color_func=tile_color_func)
        ]
...
def stim_circuit_html_viewer(
    ...
    *,
    ...
    if tile_color_func is None:
        ...
        def default_tile_color_func(tile: Tile) -> tuple[float, float, float, float]:
            if tile.basis == "X":
                return 1, 0, 0, 0.25
            elif tile.basis == "Y":
                return 0, 1, 0, 0.25
    ...
    state = _SvgState(patch, tile_color_func=tile_color_func)
