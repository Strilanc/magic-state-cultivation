...
def test_reflow():
    xx = gen.PauliMap({0: "X", 1: "X"})
    yy = gen.PauliMap({0: "Y", 1: "Y"})
    zz = gen.PauliMap({0: "Z", 1: "Z"})
    chunk1 = gen.Chunk(
        q2i={0: 0, 1: 1},
        circuit=stim.Circuit(
            """
...
def test_from_circuit_with_mpp_boundaries_simple():
    chunk = gen.Chunk.from_circuit_with_mpp_boundaries(
        stim.Circuit(
            """
        QUBIT_COORDS(1, 2) 0
        MPP X0
...
def test_chunk_mpp_init_chunk():
    chunk = gen.Chunk(
        ...
        flows=[
            gen.Flow(
                center=0,
                start=gen.PauliMap({0: "X", 1j: "Y", 2j: "Z"}),
            ),
            gen.Flow(
                center=0,
                start=gen.PauliMap({0: "Y", 1j: "X", 2j: "Z"}),
                obs_key=1,
            ),
        ],
    )
